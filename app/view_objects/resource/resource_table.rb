module Resource
  class ResourceTable < Table
    ...
    def initialize(resources, q, current_user, args)
      ...
      @namespace_url                    = args[:namespace_url]
      params = args[:params]
    ...
    def set_action_columns(resource, columns)
      ...
      columns << link_to(t('form.view'), url_for(resource_url)) if can_view?
      columns << link_to(t('form.edit'), url_for([:edit] | resource_url)) if can_edit?
      columns << link_to(t('form.delete'), url_for(resource_url), method: 'delete')  if can_delete?
    end
